...
翻译页(给图片添加标点并附上翻译)左侧展示待翻译源图的区域,对应站点 `[class*="ImageSourceViewer"]` / `[class*="Viewer"]`(`content.js` 与主题 CSS 均按此选择器定位)。深色模式下可对其中的源图做降暗(`filter: brightness`),背景保持不变,标注/批注层不受影响。
